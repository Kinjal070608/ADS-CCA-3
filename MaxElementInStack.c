...
int* getMax(int operations_count, char** operations, int* result_count) {
    ...
    int *output = malloc(operations_count * sizeof(int));
    int top = -1;
    int maxTop = -1;
    int outIndex = 0;
    for (int i = 0; i < operations_count; i++) {
        if (operations[i][0] == '1') {
            int x = atoi(operations[i] + 2);
            stack[++top] = x;
            if (maxTop == -1)
                maxStack[++maxTop] = x;
            else {
                int currentMax = maxStack[maxTop];
                maxStack[++maxTop] = (x > currentMax) ? x : currentMax;
            }
        }
            ...
            output[outIndex++] = maxStack[maxTop];
        }
    }
    *result_count = outIndex;
    return output;
}
