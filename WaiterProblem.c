...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int* A_top = malloc((q + 1) * sizeof(int));
    for (int i = 0; i <= q; i++) {
        A[i] = malloc(number_count * sizeof(int));
        A_top[i] = -1;
    }
    int** B = malloc((q + 1) * sizeof(int*));
    int* B_top = malloc((q + 1) * sizeof(int));
    for (int i = 0; i <= q; i++) {
        B[i] = malloc(number_count * sizeof(int));
        B_top[i] = -1;
    }
    for (int i = 0; i < number_count; i++) {
        A[0][++A_top[0]] = number[i];
    }
    for (int i = 1; i <= q; i++) {
        int p = primes[i - 1];
        while (A_top[i - 1] >= 0) {
            int value = A[i - 1][A_top[i - 1]--];
    ...
    for (int i = 1; i <= q; i++) {
        while (B_top[i] >= 0) {
            result[idx++] = B[i][B_top[i]--];
        }
    }
    while (A_top[q] >= 0) {
        result[idx++] = A[q][A_top[q]--];
    }
    *result_count = idx;
    return result;
}
